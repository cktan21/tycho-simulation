...
async fn main() {
    ...
    let tycho_message_processor: JoinHandle<anyhow::Result<()>> = tokio::spawn(async move {
        ...
        let mut file = std::fs::File::create("block_updates.json").expect("Failed to create file");

        // Loop through block updates
        while let Some(msg) = protocol_stream.next().await {
            let msg_clone = msg.clone(); // Clone the message for logging
            let json_str = serde_json::to_string_pretty(&msg_clone).unwrap();

            // Print to console (optional)
            println!("Received BlockUpdate: {}", json_str);

            // Write to file
            writeln!(file, "{}", json_str).expect("Failed to write to file");
